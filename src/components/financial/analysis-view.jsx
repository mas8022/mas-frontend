...
export function AnalysisView() {
  ...
  if (isPending || !data?.data) {
    return (
      <Card dir="rtl" className="bg-white dark:bg-black border-0 shadow-md text-gray-800 dark:text-gray-200">
  ...
  if (totalIncome === 0 && expensesIncome === 0) {
    return (
      <Card dir="rtl" className="bg-white dark:bg-zinc-900 border-0 shadow-md text-gray-800 dark:text-gray-200">
        ...
        <CardContent className="p-8">
          <Alert variant="default" className="bg-muted dark:bg-zinc-800 border border-border text-center">
            <AlertTitle className="text-lg max-[500px]:text-sm font-semibold text-center">
              هیچ اطلاعات مالی ثبت نشده است
            </AlertTitle>
            <AlertDescription className="text-sm text-muted-foreground mt-1">
              لطفاً ابتدا یک درآمد یا هزینه ثبت کنید تا گزارش مالی شما نمایش داده شود.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
  ...
  // ✅ بقیه‌ی گزارش مالی فقط وقتی که داده داریم
  return (
    <Card dir="rtl" className="bg-white dark:bg-transparent border-0 shadow-md text-gray-800 dark:text-gray-200">
      ...
      <CardContent className="space-y-6 p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-5">
          <div className="bg-muted dark:bg-gray-900 p-4 rounded-lg border border-border">
            <h3 className="font-bold mb-3 text-gray-800 dark:text-gray-200">بیشترین هزینه‌ها</h3>
            <div className="space-y-2 text-sm text-muted-foreground">
              {topThreeUserExpenses.map((item, i) => (
                <div key={i} className="flex justify-between">
                  <span>{item.category}</span>
                  <span className="font-bold">{formatNumber(item._sum.amount)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-muted dark:bg-gray-900 p-4 rounded-lg border border-border">
            <h3 className="font-bold mb-3 text-gray-800 dark:text-gray-200">بیشترین درآمدها</h3>
            <div className="space-y-2 text-sm text-muted-foreground">
